...
// Make the input box appear when Other is selected from the drop down menu.
document.querySelector('#title').addEventListener('change', e => {
    if (e.target.value === 'other') {
        otherJobTitle.style.display = 'initial';
    } else {
        otherJobTitle.style.display = 'none';
    }
});
...
const validMail = input => /^[^@]+@[^@.]+\.(?:com|co)$/i.test(input.value) ? input.className = 'success' : input.className = 'error';
